...
def display_data_stack():
    print(f"data stack: {data_stack} ({len(data_stack)})")
    print("         tos^\n")
    ...
    for token in tokens:
        try:
            ...
            if do_word(token):
                ...
                print(f"Error: The word '{token}' is not defined!")
                data_stack = ds_original_state
                return            
...
print("\t|                            MTKFORTH                                    |")
print("\t|                                                                        |")
print("\t| - Hint: type 'words' (without the qoutes) at the prompt to list words  |")
...
try:
    while True:
        in_buff = get_input()
        if not in_buff: continue
        do_in_buff(in_buff)
except KeyboardInterrupt:
    pass  # ctr-c will exit
